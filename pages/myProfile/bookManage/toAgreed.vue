<template>
	<view class="agree-box">
		<block v-for="item in toBeAgreeList" :key="item.id">
			<view class="to-be-Agreed" @click="goDetails(item.id)">
		...
		methods:{
			...
			getBookList(){
				this.$request.post('reservation/index/list',{
					...
					this.toBeAgreeList = this.toBeAgreeList.concat(res.result.list) 
					this.pages++
				})
			},
			// 跳转详情页
			goDetails(id){
				uni.navigateTo({
					url:'/pages/myProfile/bookManage/toAgreeDetails?id='+id
				})
			}
		}
	}
